var grainsRef = firebase.database().ref('Lists/Walmart/Grains');
var grainsList = document.getElementById("grainsList");


document.getElementById("addItemGrains").addEventListener('click', function() {
...
function refreshList(list, element) {
	...
	/*document.getElementById('grainsList').innerHTML = ls;*/
...
function edit(key, itName) {
    ...
    if (itName && itemName.length > 0) {
        ...
        var updateGrainsRef = buildEndPoint(key);
        updateGrainsRef.update({
            title: itemName
        });
    }
}
...
function buildEndPoint(key) {
	var grainsRef = firebase.database().ref('Lists/Walmart/Grains').child(key);
	location.reload();
	return grainsRef;
}

// This will get fired on inital load as well as whenever there is a change in the data.
grainsRef.on("value", function(snapshot) {
	var data = snapshot.val();
  var list = [];
	var element = 'grainsList';
  for (var key in data) {
