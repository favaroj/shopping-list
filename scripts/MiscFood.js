...
function saveToFB(itemName) {
	// save data to MiscFood Firebase
	miscFoodRef.push({
		title: itemName
	})
};
...
function refreshList(list, element) {
	...
	/*document.getElementById('miscFoodList').innerHTML = ls;*/
...
function edit(key, itName) {
    ...
    if (itemName && itemName.length > 0) {
        ...
        var updateMiscFoodRef = buildEndPoint(key);
        updateMiscFoodRef.update({
            title: itemName
        });
    }
}
...
function buildEndPoint(key) {
	var miscFoodRef = firebase.database().ref('Lists/Walmart/Misc Food').child(key);
	location.reload();
	return miscFoodRef;
}

// This will get fired on inital load as well as whenever there is a change in the data.
miscFoodRef.on("value", function(snapshot) {
	var data = snapshot.val();
  var list = [];
	var element = 'miscFoodList';
  for (var key in data) {
