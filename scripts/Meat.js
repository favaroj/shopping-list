const meatRef = firebase.database().ref('Lists/Walmart/Meat');
const meatList = document.getElementById("meatList");


document.getElementById("addItemMeat").addEventListener('click', function() {
...
function refreshList(list, element) {
	...
	/*document.getElementById('meatList').innerHTML = ls;*/
...
function edit(key, itName) {
    ...
    if (itemName && itemName.length > 0) {
        ...
        var updateMeatRef = buildEndPoint(key);
        updateMeatRef.update({
            title: itemName
        });
    }
}
...
function buildEndPoint(key) {
	var meatRef = firebase.database().ref('Lists/Walmart/Meat').child(key);
	location.reload();
	return meatRef;
}

// This will get fired on inital load as well as whenever there is a change in the data.
meatRef.on("value", function(snapshot) {
	var data = snapshot.val();
  var list = [];
	var element = 'meatList';
  for (var key in data) {
