...
function refreshList(list, element) {
	...
	/*document.getElementById('rossList').innerHTML = ls;*/
...
function edit(key, itName) {
    ...
    if (itName && itemName.length > 0) {
        ...
        var updateRossRef = buildEndPoint(key);
        updateRossRef.update({
            title: itemName
        });
    }
}
...
function buildEndPoint(key) {
	var rossRef = firebase.database().ref('Lists/Ross').child(key);
	location.reload();
	return rossRef;
}
