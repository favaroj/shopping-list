...
function edit(key, itName) {
    ...
    if (itName && itemName.length > 0) {
        ...
        var updateProduceRef = buildEndPoint(key);
        updateProduceRef.update({
            title: itemName
        });
    }
}
...
function buildEndPoint(key) {
	var produceRef = firebase.database().ref('Lists/Walmart/Produce').child(key);
	location.reload();
	return produceRef;
}

// This will get fired on inital load as well as whenever there is a change in the data.
produceRef.on("value", function(snapshot) {
	var data = snapshot.val();
  var list = [];
	var element = 'produceList';
  for (var key in data) {
