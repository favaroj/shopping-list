...
function refreshList(list, element) {
	...
	/*document.getElementById('dairyList').innerHTML = ls;*/
...
function edit(key, itName) {
    ...
    if (itName && itemName.length > 0) {
        ...
        var updateDairyRef = buildEndPoint(key);
        updateDairyRef.update({
            title: itemName
        });
    }
}
...
function buildEndPoint(key) {
	var dairyRef = firebase.database().ref('Lists/Walmart/Dairy').child(key);
	location.reload();
	return dairyRef;
}

// This will get fired on inital load as well as whenever there is a change in the data.
dairyRef.on("value", function(snapshot) {
	var data = snapshot.val();
  var list = [];
	var element = 'dairyList';
  for (var key in data) {
